...
- (void)addDotsViews
{
    ...
    for (int i = 0; i < kDotsNumber; i ++)
    {
        ...
        contextView.backgroundColor = UIColorFromRGB(0xCED7E6);
...
-(PaymentPasswordAlertVw *)paymentPasswordAlertVw{
    if (nil == _paymentPasswordAlertVw) {
        ...
        _paymentPasswordAlertVw.frame = CGRectMake(0, 0, kScreenWidth, 235);
        ...
        if ([str isEqualToString:@"zh-Hans"]) {
            _paymentPasswordAlertVw.titleLabel.text = @"请输入支付密码";
            [_paymentPasswordAlertVw.forgetPwdBtn setTitle:@"忘记密码?" forState:UIControlStateNormal];
            [_paymentPasswordAlertVw.forgetPwdBtn setTitleColor:[UIColor colorWithRed:23/255.0 green:59/255.0 blue:157/255.0 alpha:1] forState:UIControlStateNormal];
        }else{
...
-(UIView *)textFieldView{
    if (nil == _textFieldView) {
       CGFloat height = self.isNormal ? _paymentPasswordAlertVw.forgetPwdBtn.frame.origin.y - 70 : CGRectGetMaxY(_paymentPasswordAlertVw.handicapCostLabel.frame) + 30;
        _textFieldView = [[UIView alloc] initWithFrame:CGRectMake(20, height, [UIScreen mainScreen].bounds.size.width - 50 - 40,0)];
...
-(void)setPaymentPasswordAlertPrice:(NSString *)price{
    self.paymentPasswordAlertVw.priceLabel.text = price;
}

-(void)setPaymentPasswordAlertHandicapCost:(NSString *)handicapCost{
    self.paymentPasswordAlertVw.handicapCostLabel.text = handicapCost;
}

#pragma mark -设置转换数量
-(void)setNumberOfTransactions:(NSString *)TransactionsNum{
    [self.paymentPasswordAlertVw.titleLabel setHidden:YES];
    [self.paymentPasswordAlertVw.numberTransactionsTitleLabel setHidden:NO];
    [self.paymentPasswordAlertVw.numberTransactionsLabel setHidden:NO];
    self.paymentPasswordAlertVw.numberTransactionsLabel.text = TransactionsNum;
}
    
-(void)setNumberOfTransactionsTitle:(NSString *)transactionsNumTitle{
    self.paymentPasswordAlertVw.numberTransactionsTitleLabel.text = transactionsNumTitle;
}

#pragma mark -设置标题
-(void)setPaymentPasswordAlertPriceTitle:(NSString *)priceTitle{
    self.paymentPasswordAlertVw.headingLabel.text = priceTitle;
}

-(void)setPaymentPasswordAlertHandicapCostTitle:(NSString *)handicapCostTitle{
    self.paymentPasswordAlertVw.handicapCostTitleLabel.text = handicapCostTitle;
}
