...
-(void)setDataPackerWithResponse{
    QFDatePickerView *datePickerView = [[QFDatePickerView alloc]initDatePackerWithResponse:^(NSString *str) {
        if([self.delegate respondsToSelector:@selector(datePackerWithResponse:)]) {
            [self.delegate datePackerWithResponse:str];
        }
    }];
    [datePickerView show];
}
...
- (void)setViewInterface {
    ...
    //添加确定和取消按钮
    for (int i = 0; i < 2; i ++) {
        int x = i == 0 ? 15 : self.frame.size.width - 60 - 15;
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(x  , 5, 60, 30)];
        [button setTitle:i == 0 ? @"取消" : @"确定" forState:UIControlStateNormal];
        if (i == 0) {
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            button.backgroundColor = [UIColor colorWithRed:230 / 255.0 green:110 / 255.0 blue:63 / 255.0 alpha:1];
...
- (void)getCurrentDate {
    ...
    if (currentMonth < 10) {
        selectecMonth = [NSString stringWithFormat:@"0%ld",(long)currentMonth];
    }else{
        selectecMonth = [NSString stringWithFormat:@"0%ld",(long)currentMonth];
    }
}
...
- (void)setMonthArray {
    ...
    if ([[selectedYear substringWithRange:NSMakeRange(0, 4)] isEqualToString:[NSString stringWithFormat:@"%ld",currentYear]]) {
        for (NSInteger i = 1 ; i <= currentMonth; i++) {
            if (i < 10) {
                NSString *monthStr = [NSString stringWithFormat:@"0%ld月",(long)i];
